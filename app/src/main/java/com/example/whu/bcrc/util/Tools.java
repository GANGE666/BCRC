...
public class Tools {
    ...
    public static Date strs2Date(String time) {
        try {
    ...
    public static String getShowTime(String time) {
        String timeStr = "1970/01/01 00:00";
        Calendar itemCal = Calendar.getInstance();
        itemCal.set(Integer.parseInt(time.substring(0, 4)), Integer.parseInt(time.substring(5, 7)) - 1
                , Integer.parseInt(time.substring(8, 10)));
        String itemTime = itemCal.get(Calendar.YEAR) + "/" + (itemCal.get(Calendar.MONTH) + 1) + "/"
                + itemCal.get(Calendar.DAY_OF_MONTH);
        itemCal.setTime(Tools.strs2Date(itemTime));

        Calendar cal = Calendar.getInstance();
        cal.setTime(new Date());
        String dateTime = cal.get(Calendar.YEAR) + "/" + (cal.get(Calendar.MONTH) + 1) +
                "/" + cal.get(Calendar.DAY_OF_MONTH);
        Date curDate = Tools.strs2Date(dateTime);
        cal.setTime(curDate);

        if (itemCal.getTimeInMillis() - cal.getTimeInMillis() == 0) {
            timeStr = time.substring(11, 16);
        } else if (curDate.getTime() - itemCal.getTimeInMillis() == 24 * 60 * 60 * 1000) {
            timeStr = "昨天" + time.substring(11, 16);
        } else {
            timeStr = time.substring(0, 16);
        }
        return timeStr;
    }
     ...
     * @return 加密后的字符串
     * @author wangzijun 2014-12-01 11:12:00 MD5 32位加密算法
     */
    ...
    /**
     * 检查密码是否符合格式, 要求：6-18位，数字，字母，下划线，至少要数字，字母两种以上不同组合，不能含有空格
     *
     * @param pwd 输入的密码
     ...
     * DecimalFormat转换最简便
     */
    public static String getDoubleFormat(double f) {

        if (f == 0) {
            return "0.00";
        }
        if (f < 1) {
            f = new BigDecimal(f).setScale(2,
    ...
    public static String setMoney(String money) {
        String str = String.format("%.2f", Double.parseDouble(money));
        return str;
    }
