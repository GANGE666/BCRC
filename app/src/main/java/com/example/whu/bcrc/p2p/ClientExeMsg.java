			...
			try{
				if(!ClientMain.MsgQueueEmpty()){
					...
					case ClientMain.SEND_MSG:
						 ...
						 * Need to fix
						 */
						Log.d("Receive:", "\t"+ element[PosText]);
						//DeviceInfo.receiveMsg.add(element[PosText]);
						DeviceInfoActivity.receiveMsg.add(element[PosText]);

						System.out.println(element[PosText]);
